...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    ...
    >
      <p className="font-body text-[13px] text-foreground font-medium mb-1">Year {label}</p>
...
const TimelineChart = ({ points }: TimelineChartProps) => (
  <motion.div
    ...
    <div className="h-[280px]">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={points} margin={{ top: 10, right: 10, bottom: 0, left: -10 }}>
          ...
          <Area type="monotone" dataKey="invested_amount" stroke="#3B82F6" fill="url(#investedGrad2)" strokeWidth={2}
            activeDot={{ r: 4, fill: '#3B82F6', stroke: 'rgba(59,130,246,0.3)', strokeWidth: 4 }}
          />
          <Area type="monotone" dataKey="portfolio_value" stroke="#00E5A0" fill="url(#valueGrad2)" strokeWidth={2}
